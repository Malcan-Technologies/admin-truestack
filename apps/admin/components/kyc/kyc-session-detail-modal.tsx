...
export function KycSessionDetailModal({
  ...
  const saveWebhookUrl = async () => {
    if (!sessionId || !webhookUrlEdit.trim()) return;
    setSavingWebhookUrl(true);
    try {
      await apiClient(`/api/admin/kyc-sessions/${sessionId}`, {
        method: "PATCH",
        ...
        {loading ? (
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
            ...
            <TabsContent value="overview" className="mt-4 space-y-4">
              {/* Client Info Banner */}
              <div className="flex items-center gap-4 p-4 rounded-lg border border-indigo-500/30 bg-indigo-500/5">
                ...
                <Link href={`/clients/${session.client_id}`}>
                  <Button
                    variant="outline"
                    size="sm"
                    className="border-indigo-500/30 bg-transparent text-indigo-300 hover:bg-indigo-500/10"
